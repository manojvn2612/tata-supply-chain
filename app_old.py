...
def agent_answer(df: pd.DataFrame, question: str) -> Optional[str]:
    ...
    # -------------------------------
    # POLICY OPTIMIZATION TRIGGER
    # -------------------------------
    want_policy = any(w in q for w in ["policy", "optimize", "strategy", "best policy"])
    ...
    if want_policy:
        try:
            ...
            if forecast_df is None or forecast_df.empty:
                return "Forecast data unavailable for policy optimization."

            # Use first material (or can enhance later)
            forecast_values = forecast_df["Predicted Demand"].tolist()

            # Step 2: Run risk (Monte Carlo)
            if "Lead Time" not in forecast_df.columns:
                forecast_df["Lead Time"] = 10

            risk_df = run_montecarlo_risk(forecast_df)

            # Take average stockout risk
            stockout_risk = float(risk_df["Stockout Probability"].mean())

            # Step 3: Supplier risk
            supplier_df, _ = run_supplier_risk(df)

            # Pick most common risk
            supplier_risk = "MEDIUM"
            if not supplier_df.empty and "Supplier_Risk" in supplier_df.columns:
                supplier_risk = supplier_df["Supplier_Risk"].mode()[0]

            # Step 4: Run optimization
            result = optimize_policy(
                data={
                    "initial_stock": 200,
                    "forecast": forecast_values[:30] if len(forecast_values) >= 30 else forecast_values
                },
            ...
            return f"Policy optimization failed: {e}"    
...
def chat(req: ChatRequest):
    ...
    try:
        answer = chain().invoke({"question": req.question, "model_output": model_output})
    except Exception as e:
        logger.error(f"LLM error: {e}")
        answer = model_output  # Fall back to raw output if LLM fails
